...
<div style="max-width:1200px; margin:0 auto; padding:32px 20px;">
    ...
    <div style="display:flex; align-items:center; gap:8px; font-size:13px; color:var(--text-muted); margin-bottom:24px;">
        ...
        <i class="fa fa-chevron-right" style="font-size:10px;"></i>
        <a href="{{ route('products.index', ['category' => $product->category->slug]) }}" style="color:var(--text-muted);">{{ $product->category->name }}</a>
        <i class="fa fa-chevron-right" style="font-size:10px;"></i>
    ...
    <div class="product-detail-grid">
        ...
        <div class="product-detail-info">
            ...
            <div style="display:flex; align-items:center; gap:8px; margin-bottom:12px;">
                <div style="width:28px; height:28px; border-radius:50%; background:rgba(99,102,241,0.12); display:flex; align-items:center; justify-content:center; font-size:11px; font-weight:700; color:#a5b4fc;">
                ...
                @if($product->shop->is_approved)
                    <span style="font-size:11px; background:rgba(22,163,74,0.1); color:#86efac; padding:2px 8px; border-radius:20px;">
                        <i class="fa fa-circle-check"></i> Verified
                    </span>
                @endif
            </div>

            <h1 style="font-family:'Playfair Display',serif; font-size:26px; font-weight:700; margin-bottom:12px; line-height:1.3;">{{ $product->name }}</h1>

            {{-- Category --}}
            <div style="margin-bottom:16px;">
                <span style="font-size:12px; background:rgba(99,102,241,0.1); color:#a5b4fc; padding:4px 12px; border-radius:20px;">
            ...
            <div style="margin-bottom:20px;">
                @if($product->discount_price)
                    ...
                    <div style="display:flex; align-items:center; gap:10px; margin-top:4px;">
                        <span style="font-size:18px; color:var(--text-muted); text-decoration:line-through;">৳{{ number_format($product->price, 0) }}</span>
                        <span style="font-size:13px; background:rgba(239,68,68,0.1); color:#fca5a5; padding:3px 10px; border-radius:20px; font-weight:600;">
                            {{ round((($product->price - $product->discount_price) / $product->price) * 100) }}% OFF
                        </span>
                    </div>
                @else
                    <div style="font-family:'Playfair Display',serif; font-size:32px; font-weight:700; color:var(--white);">
            ...
            <div style="display:flex; gap:12px; margin-bottom:24px;">
                <button class="btn-submit" style="flex:1; margin:0; background:linear-gradient(135deg,#6366f1,#4f46e5);">
            ...
            <div style="border-top:1px solid var(--border); padding-top:20px;">
                ...
                <p style="font-size:14px; color:var(--text-muted); line-height:1.8;">{{ $product->description }}</p>
            </div>
            @endif
